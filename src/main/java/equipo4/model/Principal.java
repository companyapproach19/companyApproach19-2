...
public class Principal {
	...
	public static void moler(Lote lote) throws InterruptedException {
		...
		System.out
				.println("D�a: " + fechaActual.getDate() + "/" + fechaActual.getMonth() + "/" + fechaActual.getYear());
	...
	public static void cocinar(Lote lote) throws InterruptedException {
		...
		if (lote.isMolido()) {
			...
			Thread.sleep(3000);
			fechaActual.setDate(fechaActual.getDate() + 1);
			System.out.println("Proceso de cocci�n finalizado. D�a: " + fechaActual.getDate() + "/"
					+ fechaActual.getMonth() + "/" + fechaActual.getYear());
			lote.setCocido(true);
		}
	}
	...
	public static void fermentar(Lote lote) throws InterruptedException {
		...
		if (lote.isCocido()) {
			...
			System.out.println("Proceso de fermentaci�n finalizado. D�a: " + fechaActual.getDate() + "/"
					+ fechaActual.getMonth() + "/" + fechaActual.getYear());
			if (densidad > 1.010) {
				System.out.println("Las pruebas de densidad indican una densidad de " + densidad
						+ ", por lo que es necesario realizar una segunda fermentaci�n.");
				fermentar2(lote);
			}
	...
	public static void fermentar2(Lote lote) throws InterruptedException {
		...
		if (lote.isFermentado()) {
			...
			System.out.println("Proceso de segunda fermentaci�n finalizado. D�a: " + fechaActual.getDate() + "/"
					+ fechaActual.getMonth() + "/" + fechaActual.getYear());
	...
	public static void embotellar(Lote lote) throws InterruptedException {
		...
		if (lote.isFermentado()) {
			...
			System.out.println("Proceso de embotellado finalizado. D�a: " + fechaActual.getDate() + "/"
					+ fechaActual.getMonth() + "/" + fechaActual.getYear());
	...
	public static void main(String[] args) throws InterruptedException {
		...
		case "s":
			...
			"que va a producir si no vacia el almacen");
				...
				case "s":
					...
					cocinar(lote);
					System.out.println("Comienza la fase de fermentaci�n.");
					fermentar(lote);
		...
		case "s":
			a = new Pilsner();
			lote2 = new Lote(0, a, fechaActual);
			AlmacenLotes.almacenarLote(lote);
			System.out.println("Comienza la fase de molienda.");
			moler(lote);
			System.out.println("Comienza la fase de cocinado.");
		...
		case "s":
			...
			if (AlmacenLotes.getLista().get(i).getIdBd()-id==0) {
				System.out.println("En preparaci�n.");
				encontrado=true;
			}
			}
			}
